...
def place_smart_order_with_auth(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if res is not None and getattr(res, 'status', 200) >= 500:
                if attempt < max_retries:
                    wait_time = backoff_factor * (2**attempt)
                    logger.warning(f"Broker API returned {res.status}. Retrying in {wait_time}s...")
            ...
            if attempt < max_retries:
                wait_time = backoff_factor * (2**attempt)
                logger.warning(f"Error in broker_module.place_smartorder_api: {e}. Retrying in {wait_time}s...")
                time.sleep(wait_time)
            else:
