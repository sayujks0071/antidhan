#!/usr/bin/env python3
"""
Nifty Regime Adaptive Strategy (OpenAlgo Web UI Compatible)
High-Probability Nifty Weekly Income Strategy that adapts to market regimes.
Logic:
- Calculates PCR (Put-Call Ratio) from the option chain.
- Regime: Bullish (>1.2), Bearish (<0.8), or Neutral (0.8-1.2).
- Entry: > 10:00 AM, if Straddle Premium > 100.
- Strategy Selection:
  - Neutral: Iron Condor (Sell OTM2, Buy OTM4).
  - Bullish: Put Credit Spread (Sell OTM2 PE, Buy OTM4 PE).
  - Bearish: Call Credit Spread (Sell OTM2 CE, Buy OTM4 CE).
...
strategies_dir = os.path.dirname(script_dir) # Standard assumption
utils_dir = os.path.join(strategies_dir, "utils")

# Fix for placement in strategies/ root
if not os.path.exists(os.path.join(utils_dir, "optionchain_utils.py")):
    strategies_dir = script_dir
    utils_dir = os.path.join(strategies_dir, "utils")

sys.path.insert(0, utils_dir)

# Add project root to sys.path for 'utils' package imports in trading_utils
...
class NiftyRegimeAdaptiveStrategy:
    ...
    def _open_position(self, chain, regime, entry_reason):
        """Open Position based on Regime."""
        self.logger.info(f"Attempting to open {regime} position ({entry_reason})...")

        legs_config = []

        # Define strategy legs based on regime
        if regime == "NEUTRAL":
        ...
        for cfg in legs_config:
            ...
            if found_item:
                ...
                # Pass SYMBOL to API for precision
                api_legs.append({
                    "symbol": symbol,
        ...
        if len(resolved_legs) != len(legs_config):
            self.logger.error("Failed to resolve all legs.")
            return
