...
class MCXSmartStrategy(BaseStrategy):
    ...
    def setup(self):
        ...
        # Set default exchange/interval if not provided via CLI overrides
        if self.exchange == "NSE": # Default in BaseStrategy
            self.exchange = "MCX"
        if self.interval == "5m": # Default in BaseStrategy
            self.interval = "15m"
    ...
    def add_arguments(cls, parser):
        super().add_arguments(parser) # Add params automatically
