#!/usr/bin/env python3
"""
NSE RSI MACD Strategy V3
Enhanced with ATR Trailing Stop and ADX Trend Filter.
...
class NSERsiMacdStrategyV3(BaseStrategy):
    ...
    def cycle(self):
        ...
        if df.empty or len(df) < max(self.macd_slow, self.rsi_period, self.atr_period, self.adx_period):
        ...
        # We need the full df for ADX since calculate_adx_series expects High, Low, Close
        adx = self.calculate_adx_series(df, period=self.adx_period)
        ...
        self.logger.info(f"Price: {current_price}, RSI: {current_rsi:.2f}, MACD: {current_macd:.2f}, Signal: {current_signal:.2f}, ATR: {current_atr:.2f}, ADX: {current_adx:.2f}")
        ...
        if self.pm and self.pm.has_position():
            ...
            # Entry logic
            # Buy if MACD Crosses Above Signal AND RSI > 50 AND ADX > 25
            bullish_crossover = (macd.iloc[-2] <= signal_line.iloc[-2]) and (current_macd > current_signal)

            if bullish_crossover and current_rsi > 50 and current_adx > self.adx_threshold:
                qty = self.get_adaptive_quantity(current_price)
                self.logger.info(f"Entry signal detected (Bullish Trend, ADX > {self.adx_threshold}). Buying {qty} at {current_price}")
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--adx_period', type=int, default=14, help='ADX Period')
...
def generate_signal(df, client=None, symbol=None, params=None):
    ...
    try:
        from strategies.utils.trading_utils import calculate_rsi, calculate_macd, calculate_atr, calculate_adx
    except ImportError:
         try:
             from utils.trading_utils import calculate_rsi, calculate_macd, calculate_atr, calculate_adx
         except ImportError:
             from openalgo.strategies.utils.trading_utils import calculate_rsi, calculate_macd, calculate_atr, calculate_adx
    ...
    adx = calculate_adx(df, period=strat_params['adx_period'])

    bullish_crossover = (macd.shift(1) <= signal.shift(1)) & (macd > signal)
    bearish_crossover = (macd.shift(1) >= signal.shift(1)) & (macd < signal)

    if bullish_crossover.iloc[-1] and rsi.iloc[-1] > 50 and adx.iloc[-1] > strat_params['adx_threshold']:
        return 'BUY', 1.0, {'reason': 'MACD Crossover + RSI + ADX'}
