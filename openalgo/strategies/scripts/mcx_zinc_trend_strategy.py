...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        # Calculate ADX (simplified approximation for this script)
        ...
        tr_smooth = df["atr"] * period_atr  # Approximate smoothed TR
        plus_di = 100 * (plus_dm_series.rolling(window=period_adx).sum() / tr_smooth)
        minus_di = 100 * (minus_dm_series.rolling(window=period_adx).sum() / tr_smooth)
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            if (sma_fast > sma_slow) and (rsi > 50) and (adx > 25):
                logger.info(f"BUY SIGNAL: Price={close}, RSI={rsi:.2f}, ADX={adx:.2f}")
                if self.pm:
                    self.pm.update_position(base_qty, close, "BUY")
            elif (sma_fast < sma_slow) and (rsi < 50) and (adx > 25):
                logger.info(f"SELL SIGNAL: Price={close}, RSI={rsi:.2f}, ADX={adx:.2f}")
                if self.pm:
        ...
        # Exit Logic
            ...
            if pos_qty > 0:  # Long position
                if (sma_fast < sma_slow) or (rsi < 40):
                    logger.info("EXIT BUY: Trend Faded")
                    if self.pm:
                        self.pm.update_position(abs(pos_qty), close, "SELL")
            elif pos_qty < 0:  # Short position
                if (sma_fast > sma_slow) or (rsi > 60):
                    logger.info("EXIT SELL: Trend Faded")
                    if self.pm:
