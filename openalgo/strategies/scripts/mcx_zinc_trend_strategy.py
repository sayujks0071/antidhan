...
try:
    from trading_utils import APIClient, PositionManager, is_market_open, calculate_adx
except ImportError:
    try:
        sys.path.insert(0, strategies_dir)
        from utils.trading_utils import APIClient, PositionManager, is_market_open, calculate_adx
    except ImportError:
        try:
            from openalgo.strategies.utils.trading_utils import APIClient, PositionManager, is_market_open, calculate_adx
        except ImportError:
...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        df["sma_fast"] = df["close"].rolling(window=self.params["period_sma_fast"]).mean()
        df["sma_slow"] = df["close"].rolling(window=self.params["period_sma_slow"]).mean()
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # Buy if SMA(fast) > SMA(slow) AND RSI > rsi_buy AND ADX > adx_threshold
            if current["sma_fast"] > current["sma_slow"] and prev["sma_fast"] <= prev["sma_slow"]:
                if current["rsi"] > self.params["rsi_buy"] and current["adx"] > self.params["adx_threshold"]:
            ...
            # Sell Short if SMA(fast) < SMA(slow) AND RSI < rsi_sell AND ADX > adx_threshold
            elif current["sma_fast"] < current["sma_slow"] and prev["sma_fast"] >= prev["sma_slow"]:
                if current["rsi"] < self.params["rsi_sell"] and current["adx"] > self.params["adx_threshold"]:
        ...
        # Exit Logic
            ...
            # Exit long position
            if pos_qty > 0:
                # Exit if SMA(fast) < SMA(slow)
                if current["sma_fast"] < current["sma_slow"]:
                    logger.info(f"EXIT LONG: Trend Reversed. Price={current['close']}")
                    self.pm.update_position(abs(pos_qty), current["close"], "SELL")

                # Stop Loss based on ATR
                elif current["close"] < entry_price - (self.params["stop_loss_atr_multiplier"] * current["atr"]):
                    logger.info(f"EXIT LONG: Stop Loss hit. Price={current['close']}")
                    self.pm.update_position(abs(pos_qty), current["close"], "SELL")
            ...
            # Exit short position
                ...
                # Exit if SMA(fast) > SMA(slow)
                if current["sma_fast"] > current["sma_slow"]:
                    logger.info(f"EXIT SHORT: Trend Reversed. Price={current['close']}")
                    self.pm.update_position(abs(pos_qty), current["close"], "BUY")

                # Stop Loss based on ATR
                elif current["close"] > entry_price + (self.params["stop_loss_atr_multiplier"] * current["atr"]):
                    logger.info(f"EXIT SHORT: Stop Loss hit. Price={current['close']}")
                    self.pm.update_position(abs(pos_qty), current["close"], "BUY")
    ...
    def generate_signal(self, df):
        ...
        if current["sma_fast"] > current["sma_slow"] and prev["sma_fast"] <= prev["sma_slow"]:
            if current["rsi"] > self.params["rsi_buy"] and current["adx"] > self.params["adx_threshold"]:
                return "BUY", 1.0, {"reason": "trend_crossover_up"}

        elif current["sma_fast"] < current["sma_slow"] and prev["sma_fast"] >= prev["sma_slow"]:
            if current["rsi"] < self.params["rsi_sell"] and current["adx"] > self.params["adx_threshold"]:
                return "SELL", 1.0, {"reason": "trend_crossover_down"}
