...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        if (not seasonality_ok or not global_alignment_ok) and not has_position:
            logger.info("Seasonality or Global Alignment Weak: Skipping new entries.")
            return
        ...
        # Exit Logic
            ...
            if pos_qty > 0:
                if prev["close"] <= prev["sma"] and current["close"] > current["sma"]:
                    logger.info("EXIT LONG: SMA Crossover")
                    self.pm.update_position(abs(pos_qty), current["close"], "SELL")

            # Short position
            elif pos_qty < 0:
                if prev["close"] >= prev["sma"] and current["close"] < current["sma"]:
                    logger.info("EXIT SHORT: SMA Crossover")
    ...
    def generate_signal(self, df):
        ...
        base_qty = 1.0
        if usd_vol_high:
            base_qty = max(0.7, float(base_qty * 0.7))  # Backtesting allows fractional quantities

        # Exit Long: SMA crossover (crossing above SMA)
        if prev["close"] <= prev["sma"] and current["close"] > current["sma"]:
            return "EXIT_BUY", 0.0, {"reason": "sma_crossover_up"}

        # Exit Short: SMA crossover (crossing below SMA)
        if prev["close"] >= prev["sma"] and current["close"] < current["sma"]:
            return "EXIT_SELL", 0.0, {"reason": "sma_crossover_down"}

        # Buy: Close < Lower BB AND RSI < 30
        if current["close"] < current["lower_bb"] and current["rsi"] < 30:
            return "BUY", base_qty, {"reason": "lower_bb_rsi_os"}

        # Sell: Close > Upper BB AND RSI > 70
        if current["close"] > current["upper_bb"] and current["rsi"] > 70:
            return "SELL", base_qty, {"reason": "upper_bb_rsi_ob"}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbol", type=str, help="MCX Symbol (e.g., CRUDEOILM05FEB26FUT)")
