...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        df["sma_20"] = df["close"].rolling(window=period_bb).mean()
        df["std"] = df["close"].rolling(window=period_bb).std()
        df["upper_bb"] = df["sma_20"] + (std_dev * df["std"])
        df["lower_bb"] = df["sma_20"] - (std_dev * df["std"])
    ...
    def check_signals(self):
        ...
        # Buy if Close < Lower BB AND RSI < 30
        # Sell if Close > Upper BB AND RSI > 70
        # Exit strictly on SMA(20) crossover
        ...
        # Exit Logic
            ...
            # Exit long position on crossover below SMA 20
            if pos_qty > 0 and current["close"] > current["sma_20"] and prev["close"] <= prev["sma_20"]:
                logger.info(f"EXIT BUY: Price Crossed SMA20")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")

            # Exit short position on crossover above SMA 20
            elif pos_qty < 0 and current["close"] < current["sma_20"] and prev["close"] >= prev["sma_20"]:
                logger.info(f"EXIT SELL: Price Crossed SMA20")
    ...
    def generate_signal(self, df):
        ...
        if current["close"] < current["lower_bb"] and current["rsi"] < 30:
            return "BUY", 1.0, {"reason": "signal_triggered", "rsi": current["rsi"], "price": current["close"]}

        elif current["close"] > current["upper_bb"] and current["rsi"] > 70:
            return "SELL", 1.0, {"reason": "signal_triggered", "rsi": current["rsi"], "price": current["close"]}

        # Exits are triggered when crossing SMA20 (Need state management for exits in backtesting usually,
        # but emitting EXIT if crossing to be safe)
        elif current["close"] > current["sma_20"] and prev["close"] <= prev["sma_20"]:
            return "EXIT_BUY", 0.0, {"reason": "exit_sma_crossed", "price": current["close"]}

        elif current["close"] < current["sma_20"] and prev["close"] >= prev["sma_20"]:
            return "EXIT_SELL", 0.0, {"reason": "exit_sma_crossed", "price": current["close"]}
