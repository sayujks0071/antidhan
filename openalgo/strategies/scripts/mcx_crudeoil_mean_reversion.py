#!/usr/bin/env python3
"""
MCX Crude Oil Mean Reversion Strategy
MCX Commodity trading strategy with multi-factor analysis using Bollinger Bands and RSI
"""
import os
import sys
import time
...
try:
    ...
    try:
        ...
        try:
            ...
            is_market_open = lambda exchange='MCX', **kwargs: True

# Setup Logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
logger = logging.getLogger("MCX_CRUDEOIL_MEAN_REVERSION")
...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        period_bb = self.params.get("period_bb", 20)
        std_dev = self.params.get("std_dev", 2.0)
        df["sma"] = df["close"].rolling(window=period_bb).mean()
        df["std"] = df["close"].rolling(window=period_bb).std()
    ...
    def check_signals(self):
        ...
        # Using a base quantity larger than 1 to allow for fractional reduction
        # MCX scripts don't accept quantity via CLI arguments, so we hardcode it for now.
        base_qty = 2
        if usd_vol_high:
        ...
        # Entry Logic
        if not has_position:
            # Buy if Close < Lower Bollinger Band AND RSI < 30
            if current["close"] < current["lower_bb"] and current["rsi"] < 30:
                logger.info(f"BUY SIGNAL: Price={current['close']}, RSI={current['rsi']:.2f}, Lower BB={current['lower_bb']:.2f}")
                if self.pm:
                    self.pm.update_position(base_qty, current["close"], "BUY")

            # Sell if Close > Upper Bollinger Band AND RSI > 70
            elif current["close"] > current["upper_bb"] and current["rsi"] > 70:
                logger.info(f"SELL SIGNAL: Price={current['close']}, RSI={current['rsi']:.2f}, Upper BB={current['upper_bb']:.2f}")
                if self.pm:
        ...
        # Exit Logic
            ...
            # Exit on SMA(20) crossover (reversion to the mean)
            if pos_qty > 0 and current["close"] >= current["sma"]:
                logger.info(f"EXIT BUY: Reverted to Mean (SMA={current['sma']:.2f})")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")

            elif pos_qty < 0 and current["close"] <= current["sma"]:
                logger.info(f"EXIT SELL: Reverted to Mean (SMA={current['sma']:.2f})")
    ...
    def generate_signal(self, df):
        ...
        # Signal logic
        if current["close"] < current["lower_bb"] and current["rsi"] < 30:
            return "BUY", 1.0, {"reason": "price < lower_bb & rsi < 30"}
        elif current["close"] > current["upper_bb"] and current["rsi"] > 70:
            return "SELL", 1.0, {"reason": "price > upper_bb & rsi > 70"}

        # Determine exit signals more cautiously for backtesting
        # Only issue exit if price crosses the SMA
        prev = self.data.iloc[-2] if len(self.data) > 1 else current
        if prev["close"] < prev["sma"] and current["close"] >= current["sma"]:
            return "EXIT_BUY", 1.0, {"reason": "reverted to mean"}
        elif prev["close"] > prev["sma"] and current["close"] <= current["sma"]:
            return "EXIT_SELL", 1.0, {"reason": "reverted to mean"}
