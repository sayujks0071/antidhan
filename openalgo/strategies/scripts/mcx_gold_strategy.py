...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        # Add more indicators
        df["ema_fast"] = df["close"].ewm(span=self.params.get("period_ema_fast", 9), adjust=False).mean()
        df["ema_slow"] = df["close"].ewm(span=self.params.get("period_ema_slow", 21), adjust=False).mean()
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # Entry condition: EMA Crossover and RSI not overbought
            if prev["ema_fast"] <= prev["ema_slow"] and current["ema_fast"] > current["ema_slow"] and current["rsi"] < 70:
                logger.info(f"BUY SIGNAL: Price={current['close']}, RSI={current['rsi']:.2f}")
                if self.pm:
                    self.pm.update_position(base_qty, current["close"], "BUY")
            # Entry condition for SHORT: EMA Crossover and RSI not oversold
            elif prev["ema_fast"] >= prev["ema_slow"] and current["ema_fast"] < current["ema_slow"] and current["rsi"] > 30:
        ...
        # Exit Logic
            ...
            # Exit condition: EMA crossover opposite to position
            if pos_qty > 0 and prev["ema_fast"] >= prev["ema_slow"] and current["ema_fast"] < current["ema_slow"]:
                logger.info(f"EXIT LONG: Trend Faded")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")
            elif pos_qty < 0 and prev["ema_fast"] <= prev["ema_slow"] and current["ema_fast"] > current["ema_slow"]:
                logger.info(f"EXIT SHORT: Trend Faded")
    ...
    def generate_signal(self, df):
        ...
        # Signal logic
        if prev["ema_fast"] <= prev["ema_slow"] and current["ema_fast"] > current["ema_slow"] and current["rsi"] < 70:
            return "BUY", 1.0, {"reason": "ema_crossover_up"}
        elif prev["ema_fast"] >= prev["ema_slow"] and current["ema_fast"] < current["ema_slow"] and current["rsi"] > 30:
            return "SELL", 1.0, {"reason": "ema_crossover_down"}
