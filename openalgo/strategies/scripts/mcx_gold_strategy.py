...
logger = logging.getLogger("MCX_GOLD_STRATEGY")
...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        # SMA 20 and 50
        df["sma_20"] = df["close"].rolling(window=self.params.get("period_sma_fast", 20)).mean()
        df["sma_50"] = df["close"].rolling(window=self.params.get("period_sma_slow", 50)).mean()
        ...
        df['tr_smoothed'] = df['tr'].rolling(window=period_adx).sum()
        df['+dm_smoothed'] = df['+dm'].rolling(window=period_adx).sum()
        df['-dm_smoothed'] = df['-dm'].rolling(window=period_adx).sum()
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # Buy if SMA(20) > SMA(50) AND RSI > 50 AND ADX > 25
            if current["sma_20"] > current["sma_50"] and current["rsi"] > 50 and current["adx"] > adx_threshold:
            ...
            # Sell if SMA(20) < SMA(50) AND RSI < 50 AND ADX > 25
            elif current["sma_20"] < current["sma_50"] and current["rsi"] < 50 and current["adx"] > adx_threshold:
        ...
        # Exit Logic
            ...
            if pos_qty > 0:
                # Long Exit: SMA(20) < SMA(50)
                if current["sma_20"] < current["sma_50"]:
                    logger.info(f"EXIT LONG: Trend Reversed")
                    self.pm.update_position(abs(pos_qty), current["close"], "SELL")
            elif pos_qty < 0:
                # Short Exit: SMA(20) > SMA(50)
                if current["sma_20"] > current["sma_50"]:
                    logger.info(f"EXIT SHORT: Trend Reversed")
    ...
    def generate_signal(self, df):
        ...
        if current["sma_20"] > current["sma_50"] and current["rsi"] > 50 and current["adx"] > adx_threshold:
            return "BUY", 1.0, {"reason": "bullish_trend_strong", "rsi": current["rsi"], "adx": current["adx"]}

        elif current["sma_20"] < current["sma_50"] and current["rsi"] < 50 and current["adx"] > adx_threshold:
            return "SELL", 1.0, {"reason": "bearish_trend_strong", "rsi": current["rsi"], "adx": current["adx"]}
