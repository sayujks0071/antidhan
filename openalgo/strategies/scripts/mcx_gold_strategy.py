...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        # Moving Averages
        period_ema_fast = self.params.get("period_ema_fast", 9)
        period_ema_slow = self.params.get("period_ema_slow", 21)
        df["ema_fast"] = df["close"].ewm(span=period_ema_fast, adjust=False).mean()
        df["ema_slow"] = df["close"].ewm(span=period_ema_slow, adjust=False).mean()

        # ADX (using numpy vectorized operations for speed)
        ...
        df["+dm"] = np.where(plus_dm_condition, plus_dm, 0.0)
        df["-dm"] = np.where(minus_dm_condition, minus_dm, 0.0)

        tr_smooth = df["atr"] * period_atr # Approximate smooth TR
        plus_di = 100 * (df["+dm"].rolling(window=period_adx).sum() / tr_smooth)
        minus_di = 100 * (df["-dm"].rolling(window=period_adx).sum() / tr_smooth)
    ...
    def check_signals(self):
        ...
        usd_vol_high = self.params.get("usd_inr_volatility", 0.0) > 1.0
        ...
        # Entry Logic (Momentum: Fast EMA > Slow EMA, RSI > 50, ADX > 25)
        if not has_position:
            buy_condition = (current["ema_fast"] > current["ema_slow"]) and \
        ...
        # Exit Logic (Fast EMA < Slow EMA, or RSI drops below 40)
        elif has_position:
            ...
            if exit_condition:
                logger.info(f"EXIT: Trend Faded or RSI dropped below 40")
    ...
    def generate_signal(self, df):
        ...
        exit_condition = (current["ema_fast"] < current["ema_slow"]) and \
                         (prev["ema_fast"] >= prev["ema_slow"])

        if buy_condition:
            return "BUY", 1.0, {"reason": "signal_triggered"}
        elif exit_condition:
            return "EXIT_BUY", 0.0, {"reason": "trend_faded"}
