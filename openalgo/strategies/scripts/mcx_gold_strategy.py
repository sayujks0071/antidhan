...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # BUY if Fast EMA crosses above Slow EMA AND RSI > 55
            if current["ema_fast"] > current["ema_slow"] and prev["ema_fast"] <= prev["ema_slow"] and current["rsi"] > 55:
        ...
        # Exit Logic
            ...
            # EXIT if Fast EMA crosses below Slow EMA OR RSI < 40
            if current["ema_fast"] < current["ema_slow"] and prev["ema_fast"] >= prev["ema_slow"] or current["rsi"] < 40:
    ...
    def generate_signal(self, df):
        ...
        if current["ema_fast"] > current["ema_slow"] and prev["ema_fast"] <= prev["ema_slow"] and current["rsi"] > 55:
            return "BUY", 1.0, {"reason": "signal_triggered", "rsi": current["rsi"], "ema_fast": current["ema_fast"]}

        # Exit condition check
        if current["ema_fast"] < current["ema_slow"] and prev["ema_fast"] >= prev["ema_slow"] or current["rsi"] < 40:
            return "EXIT_BUY", 0.0, {"reason": "trend_faded", "rsi": current["rsi"], "ema_fast": current["ema_fast"]}
