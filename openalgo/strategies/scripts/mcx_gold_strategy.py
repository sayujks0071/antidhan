...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        # ADX Approximation (Wilder's Smoothing)
        ...
        df['+dm'] = pd.Series(plus_dm, index=df.index)
        df['-dm'] = pd.Series(minus_dm, index=df.index)

        df['tr_14'] = true_range.rolling(window=period_adx).sum()
        df['+di'] = 100 * (df['+dm'].rolling(window=period_adx).sum() / df['tr_14'])
        df['-di'] = 100 * (df['-dm'].rolling(window=period_adx).sum() / df['tr_14'])
    ...
    def check_signals(self):
        ...
        # Exit Logic
            ...
            # Exit on SMA crossover or ATR-based SL/TP
            sl_dist = 1.5 * atr
            tp_dist = 3.0 * atr
            ...
            if pos_qty > 0: # Long Position
                ...
                elif current["close"] < (entry_price - sl_dist):
                    exit_signal = True
                    reason = "Stop Loss"
                elif current["close"] > (entry_price + tp_dist):
                    exit_signal = True
                    reason = "Take Profit"
            else: # Short Position
                if sma_20 > sma_50:
    ...
    def generate_signal(self, df):
        ...
        # Signal logic
        if sma_20 > sma_50 and rsi > 50 and adx > 25:
            return "BUY", 1.0, {"reason": "Trend Following (Bullish)"}
        elif sma_20 < sma_50 and rsi < 50 and adx > 25:
            return "SELL", 1.0, {"reason": "Trend Following (Bearish)"}
