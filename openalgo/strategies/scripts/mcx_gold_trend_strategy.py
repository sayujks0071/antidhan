...
try:
    ...
    try:
        ...
        try:
            ...
            def calculate_sma(series, period=20): return pd.Series()
...
logger = logging.getLogger("MCX_Gold_Trend")
...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        df["sma_fast"] = calculate_sma(df["close"], self.params.get("period_sma_fast", 20))
        df["sma_slow"] = calculate_sma(df["close"], self.params.get("period_sma_slow", 50))
        df["adx"] = calculate_adx(df, self.params.get("period_adx", 14))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MCX Gold Trend Strategy")
    parser.add_argument("--symbol", type=str, help="MCX Symbol (e.g., GOLDM05FEB26FUT)")
    parser.add_argument("--underlying", type=str, help="Commodity Name (e.g., GOLD)")
