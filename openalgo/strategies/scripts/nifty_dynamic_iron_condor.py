#!/usr/bin/env python3
"""
[Nifty Dynamic Iron Condor] - NIFTY Options (OpenAlgo Web UI Compatible)
Adaptive strategy that trades Iron Condors in range-bound markets and Credit Spreads in trending markets.
Uses EMA(20) for Trend and PCR for Sentiment.
"""
import os
import sys
import time
...
if not API_KEY:
    # Fallback for testing or if env var missing
    # In production, this should raise ValueError
...
PCR_BULLISH = safe_float(os.getenv("PCR_BULLISH", "1.1"))
...
SL_PCT = safe_float(os.getenv("SL_PCT", "40.0"))   # Exit if loss > 40% of max profit
TP_PCT = safe_float(os.getenv("TP_PCT", "50.0"))   # Exit if gain > 50% of max profit
    ...
    def _open_position(self, chain, regime, reason):
        ...
        for cfg in legs_config:
            ...
            if not found_item and offset == "OTM4":
                 # Fallback to OTM3 if OTM4 not available
                 for item in chain:
                    opt = item.get(otype, {})
        ...
        while True:
            try:
                ...
                if datetime.now().date() != self.current_date:
                    self.entered_today = False
                    self.current_date = datetime.now().date()
                    # Reset limiter daily count? TradeLimiter handles date check automatically now.
                    # But if we want to reset limiter explicitly:
                    # self.limiter = TradeLimiter(...)
