...
Dynamically switches between Iron Condor (Neutral), Bull Put Spread (Bullish), and Bear Call Spread (Bearish).
Logic:
- Regime Detection: Uses EMA(20) Trend + PCR Sentiment (>1.0 Bullish, <0.8 Bearish).
- Filters: OI Walls (Support/Resistance) to avoid buying into resistance or selling into support.
- Risk: Trailing Stop Loss (Breakeven at 25% profit, +20% locked at 40% profit).
- Time: Entry 09:30-14:30. Exit 15:15. Max Hold 45 mins.
"""
import os
import sys
import time
...
class NetCreditTracker(OptionPositionTracker):
    """
    Custom Position Tracker for Credit Strategies (Selling Options).
    ...
    def should_exit(self, chain):
        ...
        effective_sl = -self.sl_pct

        # If Profit > 40%, lock in 20%
        if self.highest_pnl_pct >= 40.0:
            effective_sl = 20.0
        # If Profit > 25%, move SL to Breakeven (0%)
        elif self.highest_pnl_pct >= 25.0:
            effective_sl = 0.0

        # Check SL (Trailing)
        if pnl_pct <= effective_sl:
            reason = "stop_loss"
            if effective_sl > -self.sl_pct:
                reason = "trailing_stop"
            return True, self.open_legs, f"{reason} ({pnl_pct:.1f}%)"
...
class NiftySmartMultiRegime:
    ...
    def update_ema(self):
        ...
        try:
            # Fetch last 3 days of 5m data to ensure enough for EMA(20)
    ...
    def analyze_chain(self, chain):
        """Calculate PCR and find Max OI Strikes (Support/Resistance)."""
        total_ce_oi = 0
        total_pe_oi = 0
        max_ce_oi = 0
        max_pe_oi = 0
        ...
        return {
            "pcr": pcr,
            "res_strike": res_strike,
            "sup_strike": sup_strike
        }

    def determine_regime(self, pcr, spot, stats):
        """
        Determine market regime based on PCR, Price vs EMA, and OI Walls.
        """
        ...
        sup = stats["sup_strike"]
        res = stats["res_strike"]

        # Filter: Avoid Bullish if right below Resistance
        # Filter: Avoid Bearish if right above Support

        # Bullish: Trending Up + PCR Support + Not at Resistance
        if (pcr > PCR_BULLISH and ema_bullish and spot < (res - OI_WALL_BUFFER)):
            return "BULLISH"

        # Bearish: Trending Down + PCR Weakness + Not at Support
        elif (pcr < PCR_BEARISH and ema_bearish and spot > (sup + OI_WALL_BUFFER)):
            return "BEARISH"

        # Neutral: PCR Balanced, Spot inside Walls
        # Or trending but blocked by walls
        elif (spot > (sup + OI_WALL_BUFFER) and spot < (res - OI_WALL_BUFFER)):
            return "NEUTRAL"
        ...
        while True:
            try:
                ...
                if datetime.now().date() != self.current_date:
                    self.current_date = datetime.now().date()
                    # Limiter resets automatically if configured or we can recreate it?
                    # TradeLimiter usually tracks count but reset logic might be internal or missing.
                    # Re-instantiating limiter daily is safer.
                    self.limiter = TradeLimiter(
                        max_per_day=MAX_ORDERS_PER_DAY,
                        max_per_hour=MAX_ORDERS_PER_HOUR,
                        cooldown_seconds=COOLDOWN_SECONDS
                ...
                if not self.tracker.open_legs:
                    ...
                    if start_time_dt <= now.time() <= end_time_dt:
                        if self.limiter.allow():
                            ...
                            stats = self.analyze_chain(chain)
                            regime = self.determine_regime(stats["pcr"], underlying_ltp, stats)
                            ...
                            should_enter = (regime != "UNKNOWN") and (regime != "WAIT") and (straddle_premium > MIN_STRADDLE_PREMIUM)

                            if self.debouncer.edge("entry_signal", should_enter):
                                self._open_position(chain, regime, f"regime_{regime}_pcr_{stats['pcr']:.2f}")
