...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        std_dev_bb = self.params.get("std_dev_bb", 2)
        df["sma_bb"] = df["close"].rolling(window=period_bb).mean()
        df["std_bb"] = df["close"].rolling(window=period_bb).std()
        df["upper_bb"] = df["sma_bb"] + (df["std_bb"] * std_dev_bb)
        df["lower_bb"] = df["sma_bb"] - (df["std_bb"] * std_dev_bb)

        # SMA for crossover exit
        period_sma_exit = self.params.get("period_sma_exit", 20)
        df["sma_exit"] = df["close"].rolling(window=period_sma_exit).mean()
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # Mean Reversion on NATURALGAS. Buy if Close < Lower Bollinger Band(20, 2) AND RSI(14) < 30.
        ...
        # Exit Logic
            ...
            # Exit on SMA(20) crossover
            if pos_qty > 0 and current["close"] > current["sma_exit"] and prev["close"] <= prev["sma_exit"]:
                logger.info(f"EXIT LONG: Price crossed above SMA(20)")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")

            elif pos_qty < 0 and current["close"] < current["sma_exit"] and prev["close"] >= prev["sma_exit"]:
                logger.info(f"EXIT SHORT: Price crossed below SMA(20)")
    ...
    def generate_signal(self, df):
        ...
        # Your signal logic
        if current["close"] < current["lower_bb"] and current["rsi"] < 30:
            return "BUY", 1.0, {"reason": "oversold_bb_rsi"}

        elif current["close"] > current["upper_bb"] and current["rsi"] > 70:
            return "SELL", 1.0, {"reason": "overbought_bb_rsi"}

        # Exit on SMA crossover
        if current["close"] > current["sma_exit"] and prev["close"] <= prev["sma_exit"]:
            return "EXIT", 0.0, {"reason": "sma_crossover"}
        elif current["close"] < current["sma_exit"] and prev["close"] >= prev["sma_exit"]:
            return "EXIT", 0.0, {"reason": "sma_crossover"}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbol", type=str, help="MCX Symbol (e.g., NATURALGAS24FEB26FUT)")
