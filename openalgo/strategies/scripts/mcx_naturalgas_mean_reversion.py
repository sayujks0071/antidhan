...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        # Exit Logic
            ...
            # Sell if Close > Upper Bollinger Band AND RSI > 70. Exit on SMA(20) crossover.
            if (current["close"] > current["upper_bb"] and current["rsi"] > 70) or current["close"] > current["sma"]:
                logger.info(f"EXIT: Trend Faded / Reverted to Mean")
                if self.pm:
    ...
    def generate_signal(self, df):
        ...
        # Signal logic
        if current["close"] < current["lower_bb"] and current["rsi"] < 30:
            return "BUY", 1.0, {"reason": "signal_triggered", "rsi": current["rsi"], "lower_bb": current["lower_bb"]}
        elif (current["close"] > current["upper_bb"] and current["rsi"] > 70) or current["close"] > current["sma"]:
            return "SELL", 1.0, {"reason": "exit_condition", "rsi": current["rsi"], "sma": current["sma"]}
