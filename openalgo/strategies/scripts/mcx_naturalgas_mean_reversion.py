#!/usr/bin/env python3
"""
MCX Natural Gas Mean Reversion
...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        df['upper_bb'] = df['sma_bb'] + (std_dev * df['std_bb'])
        df['lower_bb'] = df['sma_bb'] - (std_dev * df['std_bb'])
    ...
    def check_signals(self):
        ...
        # Exit Logic
            ...
            # Exit if Close > Middle Bollinger Band (SMA 20) for Buy positions.
            if pos_qty > 0 and current['close'] > current['sma_bb']:
                logger.info(f"EXIT BUY: Reverted to mean")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")

            # Exit if Close < Middle Bollinger Band (SMA 20) for Sell positions.
            elif pos_qty < 0 and current['close'] < current['sma_bb']:
                logger.info(f"EXIT SELL: Reverted to mean")
    ...
    def generate_signal(self, df):
        ...
        # Buy if Close < Lower Bollinger Band AND RSI < 30.
        if current['close'] < current['lower_bb'] and current['rsi'] < 30:
            return "BUY", 1.0, {"reason": "mean_reversion_buy"}

        # Sell if Close > Upper Bollinger Band AND RSI > 70.
        elif current['close'] > current['upper_bb'] and current['rsi'] > 70:
            return "SELL", 1.0, {"reason": "mean_reversion_sell"}

        # Exit if Close > Middle Bollinger Band (SMA 20) for Buy positions.
        elif current['close'] > current['sma_bb']:
             return "EXIT_BUY", 1.0, {"reason": "mean_reversion_exit_buy"}

        # Exit if Close < Middle Bollinger Band (SMA 20) for Sell positions.
        elif current['close'] < current['sma_bb']:
             return "EXIT_SELL", 1.0, {"reason": "mean_reversion_exit_sell"}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--period_bb", type=int, default=20, help="Bollinger Bands Period")
    parser.add_argument("--std_dev", type=float, default=2.0, help="Bollinger Bands Std Dev")
