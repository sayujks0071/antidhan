...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # Buy if Close < Lower Bollinger Band(20, 2) AND RSI(14) < 30
            ...
            # Sell (Short) if Close > Upper Bollinger Band AND RSI > 70
        ...
        # Exit Logic
            ...
            if pos_qty > 0 and current['close'] > current['sma']:
                logger.info(f"EXIT LONG: Price crossed above SMA. Price={current['close']}")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")
            elif pos_qty < 0 and current['close'] < current['sma']:
                logger.info(f"EXIT SHORT: Price crossed below SMA. Price={current['close']}")
