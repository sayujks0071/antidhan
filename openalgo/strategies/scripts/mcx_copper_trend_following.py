#!/usr/bin/env python3
"""
MCX Copper Trend Following Strategy
MCX Commodity trading strategy using EMA, RSI, and ATR for trend following on Copper.
"""
import os
import sys
import time
...
# Import Indicators separately for robustness
try:
    from trading_utils import calculate_ema, calculate_rsi, calculate_atr
except ImportError:
    try:
        from utils.trading_utils import calculate_ema, calculate_rsi, calculate_atr
    except ImportError:
        try:
            from openalgo.strategies.utils.trading_utils import calculate_ema, calculate_rsi, calculate_atr
        except ImportError:
...
logger = logging.getLogger("MCX_Copper_Strategy")
...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        # Calculate indicators using separate/fallback functions
        df["ema"] = calculate_ema(df["close"], period=self.params["period_ema"])
    ...
    def check_signals(self):
        ...
        if not has_position:
            ...
            if close > ema and rsi > 55:
                logger.info(f"BUY SIGNAL: Price={close}, EMA={ema:.2f}, RSI={rsi:.2f}")
                if self.pm:
                    self.pm.update_position(base_qty, close, "BUY")
            # Short Entry
            elif close < ema and rsi < 45:
                logger.info(f"SELL SIGNAL: Price={close}, EMA={ema:.2f}, RSI={rsi:.2f}")
                if self.pm:
        ...
        # Exit Logic
            ...
            if pos_qty > 0: # Long Position
                ...
                # Exit if trend reverses (Close < EMA) or Stop Loss hit
                if close < ema or close < stop_loss_price:
                    reason = "Trend Reversal" if close < ema else "Stop Loss"
                    logger.info(f"EXIT (Long): {reason}. Price={close}, Entry={entry_price}, SL={stop_loss_price:.2f}")
                    self.pm.update_position(abs(pos_qty), close, "SELL")
                ...
                if close > ema or close > stop_loss_price:
                    reason = "Trend Reversal" if close > ema else "Stop Loss"
                    logger.info(f"EXIT (Short): {reason}. Price={close}, Entry={entry_price}, SL={stop_loss_price:.2f}")
                    self.pm.update_position(abs(pos_qty), close, "BUY")
    ...
    def generate_signal(self, df):
        ...
        # Multi-Factor Check for Signal Generation (Consistent with check_signals)
        seasonality_ok = self.params.get("seasonality_score", 50) > 40
        ...
        # Signal Logic
        if close > ema and rsi > 55:
            return "BUY", 1.0, {"reason": "Trend Following Long"}
        elif close < ema and rsi < 45:
            return "SELL", 1.0, {"reason": "Trend Following Short"}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MCX Copper Trend Following Strategy")
    parser.add_argument("--symbol", type=str, help="MCX Symbol (e.g., COPPERM05FEB26FUT)")
