...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        # Entry Logic: Close > SMA 20 AND RSI > 50 AND ADX > 25
        buy_condition = (
        ...
        # Exit Logic: Close < SMA 20 OR RSI < 40
        elif has_position:
            pos_qty = self.pm.position
            entry_price = self.pm.entry_price

            exit_condition = (
                current['close'] < current['sma_20'] or
                current['rsi'] < 40
            )

            if exit_condition:
                logger.info(f"EXIT: Trend Faded. Price={current['close']}, SMA={current['sma_20']:.2f}, RSI={current['rsi']:.2f}")
    ...
    def generate_signal(self, df):
        ...
        if buy_condition:
            return "BUY", 1.0, {"reason": "Trend Breakout confirmed"}
