#!/usr/bin/env python3
"""
MCX Custom Multi-Factor Strategy
MCX Commodity trading strategy with multi-factor analysis using MACD, RSI, and Bollinger Bands.
"""
import os
import sys
import time
...
try:
    ...
    try:
        ...
        try:
            ...
            is_market_open = lambda exchange='MCX': True
...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            # Buy condition: Price near lower BB, RSI oversold (< 35), MACD crosses above Signal
            buy_condition = (
                current["close"] <= current["lower_bb"] and
                current["rsi"] < 35 and
                current["macd_line"] > current["macd_signal_line"] and
                prev["macd_line"] <= prev["macd_signal_line"]
            )
            # Sell condition: Price near upper BB, RSI overbought (> 65), MACD crosses below Signal
            sell_condition = (
                current["close"] >= current["upper_bb"] and
                current["rsi"] > 65 and
                current["macd_line"] < current["macd_signal_line"] and
                prev["macd_line"] >= prev["macd_signal_line"]
            )
        ...
        # Exit Logic
            ...
            stop_loss = entry_price - (1.5 * current["atr"]) if pos_qty > 0 else entry_price + (1.5 * current["atr"])
            take_profit = entry_price + (3.0 * current["atr"]) if pos_qty > 0 else entry_price - (3.0 * current["atr"])

            if pos_qty > 0: # Long position
                if current["close"] <= stop_loss or current["close"] >= take_profit or current["macd_line"] < current["macd_signal_line"]:
                    exit_signal = True
            ...
            if exit_signal:
                logger.info(f"EXIT: Trend Faded or Risk Parameters Hit")
                if self.pm:
    ...
    def generate_signal(self, df):
        ...
        if buy_condition:
            ...
            return "SELL", 1.0, {"reason": "signal_triggered"}
