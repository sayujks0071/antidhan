...
Iron Condor strategy for Nifty that enters after 10 AM when straddle premium > 120, sells OTM2, buys OTM4.
"""
import os
import sys
import time
    ...
    def _open_position(self, chain, reason):
        ...
        try:
            ...
            if res.get("status") == "success":
                ...
                # OptionPositionTracker tracks SL/TP natively based on legs added
        ...
        while True:
            try:
                ...
                if self.tracker.open_legs:
                    ...
                    # Additional check for MAX_HOLD_MIN since native tracker checks against self.entry_time
                    if not exit_now and self.entry_time:
                        minutes_held = (datetime.now() - self.entry_time).total_seconds() / 60
                        if minutes_held >= MAX_HOLD_MIN:
                            exit_now = True
                            exit_reason = f"time_stop ({int(minutes_held)}m)"
                ...
                if not self.tracker.open_legs and self.can_trade():
                    ...
                    if start_time_dt <= now.time() < eod_time:
                        ...
                        if self.debouncer.edge("entry_signal", should_enter):
                            self._open_position(chain, f"straddle_{straddle_premium:.2f}")
