#!/usr/bin/env python3
"""
[Nifty Hybrid Options] - NIFTY Options (OpenAlgo Web UI Compatible)
...
# and ensures utils_dir is in path for trading_utils import itself
...
# CONFIGURATION
# ===========================
STRATEGY_NAME = os.getenv("STRATEGY_NAME", "NiftyHybridOptions")
...
class NiftyHybridOptionsStrategy:
    def __init__(self):
    ...
    def can_trade(self):
        ...
        if self.entered_today and self.limiter.trades_today >= MAX_ORDERS_PER_DAY:
            return False
        ...
        while True:
            try:
                ...
                if datetime.now().date() != self.current_date:
                    self.entered_today = False
                    self.current_date = datetime.now().date()
                    # Re-init limiter to clear counters
                    self.limiter = TradeLimiter(
                        max_per_day=MAX_ORDERS_PER_DAY,
                        max_per_hour=MAX_ORDERS_PER_HOUR,
                        cooldown_seconds=COOLDOWN_SECONDS
                ...
                try:
                    ...
                    # If is_market_open fails (e.g. import error), assume open for safety in dev,
                    # but typically we sleep.
                    pass
                ...
                if self.tracker.open_legs:
                    ...
                    if now_ist.tzinfo is None:
                         now_ist = datetime.now() # Fallback

                    eod_time = datetime.strptime(EXIT_TIME, "%H:%M").time()

                    # Convert now to time for comparison (Use IST)
                    if now_ist.time() >= eod_time:
                        exit_now = True
                ...
                if not self.tracker.open_legs and self.can_trade():
                    ...
                    if start_time_dt <= now_time <= end_time_dt:
...
if __name__ == "__main__":
    strategy = NiftyHybridOptionsStrategy()
    strategy.run()
