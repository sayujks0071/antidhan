#!/usr/bin/env python3
"""
MCX Silver Mean Reversion Strategy
...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        # Exit Logic: Close > Upper BB OR RSI > 70
            ...
            if current["close"] > current["upper_bb"] or current["rsi"] > 70:
                logger.info(f"EXIT: Trend Faded or Reversing")
                if self.pm:
    ...
    def generate_signal(self, df):
        ...
        # Check Multi-factor before generating new signals
        seasonality_ok = self.params.get("seasonality_score", 50) > 40
        global_alignment_ok = self.params.get("global_alignment_score", 50) >= 40

        # Your signal logic
        if current["close"] < current["lower_bb"] and current["rsi"] < 30:
            if not seasonality_ok or not global_alignment_ok:
                 return "HOLD", 0.0, {"reason": "Filters Weak"}
            return "BUY", 1.0, {"reason": "mean_reversion_entry"}
        elif current["close"] > current["upper_bb"] or current["rsi"] > 70:
            return "SELL", 1.0, {"reason": "mean_reversion_exit"}
