...
class MCXStrategy:
    ...
    def calculate_indicators(self):
        ...
        atr_adx = pd.Series(tr).rolling(window=period_adx).sum()
        plus_di = 100 * (pd.Series(plus_dm).rolling(window=period_adx).sum() / atr_adx)
        minus_di = 100 * (pd.Series(minus_dm).rolling(window=period_adx).sum() / atr_adx)
    ...
    def check_signals(self):
        ...
        # Entry Logic
        if not has_position:
            if current['sma_fast'] > current['sma_slow'] and current['rsi'] > 50 and current['adx'] > 25:
                logger.info(f"BUY SIGNAL: Price={current['close']}, RSI={current['rsi']:.2f}")
                if self.pm:
                    self.pm.update_position(base_qty, current["close"], "BUY")
            elif current['sma_fast'] < current['sma_slow'] and current['rsi'] < 50 and current['adx'] > 25:
        ...
        # Exit Logic
            ...
            if pos_qty > 0 and (current['sma_fast'] < current['sma_slow'] or current['rsi'] < 40):
                logger.info(f"EXIT LONG: Trend Faded")
                self.pm.update_position(abs(pos_qty), current["close"], "SELL")
            elif pos_qty < 0 and (current['sma_fast'] > current['sma_slow'] or current['rsi'] > 60):
    ...
    def generate_signal(self, df):
        ...
        # Your signal logic
        if current['sma_fast'] > current['sma_slow'] and current['rsi'] > 50 and current['adx'] > 25:
            return "BUY", 1.0, {"reason": "trend_up", "rsi": current['rsi'], "adx": current['adx']}
        elif current['sma_fast'] < current['sma_slow'] and current['rsi'] < 50 and current['adx'] > 25:
            return "SELL", 1.0, {"reason": "trend_down", "rsi": current['rsi'], "adx": current['adx']}
