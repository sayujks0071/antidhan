...
def main():
    ...
    # if tested on a single symbol without parameter arrays, portfolio metrics like
    # pf.total_return() may return a scalar numpy.float64 rather than a pandas object
    # with an .index attribute. Code must check hasattr(total_returns, 'index').

    if hasattr(total_returns, 'index'):
        # Usually happens if multiple symbols or params are used
        if isinstance(total_returns.index, pd.MultiIndex):
            ...
            val = total_returns.iloc[0] if len(total_returns) > 0 else 0.0
    else:
        # Scalar value like numpy.float64
        val = total_returns
