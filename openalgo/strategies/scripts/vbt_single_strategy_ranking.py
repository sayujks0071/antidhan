...
def main():
    ...
    # In vectorbt 0.28+, we pass period directly, ticker_kwargs for yfinance init
    ...
    # natively returns a scalar float (e.g., numpy.float64) for total_return()
    total_return_val = pf.total_return()

    # Convert to float just in case, though it should be a scalar float already
    ret_pct = float(total_return_val) * 100
