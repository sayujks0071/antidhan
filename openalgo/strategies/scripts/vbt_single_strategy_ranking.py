import vectorbt as vbt
import pandas as pd
from datetime import datetime, timedelta

def main():
    # Backtest a single strategy on a single symbol
    symbol = "BTC-USD"
    print(f"Downloading data for {symbol}...")

    # For VectorBT 0.28+:
    # ticker_kwargs must contain valid yfinance.Ticker parameters (like session)
    # period should be passed directly to download()
    data = vbt.YFData.download(
        symbol,
        period="1y",
        missing_index="drop",
        ticker_kwargs={"session": None}
    )
    price = data.get("Close")

    print("Running Dual SMA Crossover Backtest...")
    fast_window = 10
    slow_window = 50

    fast_ma = vbt.MA.run(price, fast_window)
    slow_ma = vbt.MA.run(price, slow_window)

    entries = fast_ma.ma_crossed_above(slow_ma)
    exits = fast_ma.ma_crossed_below(slow_ma)

    # Need freq="1D" to prevent UserWarnings
    pf = vbt.Portfolio.from_signals(
        price,
        entries,
        exits,
        init_cash=10000,
        freq="1D"
    )

    # For a single symbol, total_return() returns a float
    total_return = pf.total_return()
    ret_pct = total_return * 100

    # Rank the strategy
    if ret_pct > 50:
        rank = "Premium"
    elif ret_pct >= 10:
        rank = "Moderate"
    else:
        rank = "Low"

    print("\n--- Strategy Performance Ranking ---")
    print(f"Symbol: {symbol}")
    print(f"Total Return: {ret_pct:.2f}%")
    print(f"Rank: {rank}")
    print("-" * 30)

if __name__ == "__main__":
    main()
