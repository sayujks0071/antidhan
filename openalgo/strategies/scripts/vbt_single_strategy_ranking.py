...
def main():
    ...
    # Note: For a single symbol parameterless strategy, total_return() natively returns
    # a scalar float (e.g. numpy.float64) in VectorBT 0.28+
    ...
    if isinstance(total_returns, (float, np.floating)):
        ...
        # Fallback in case of multi-dimensional or pandas Series
        ret_pct = float(total_returns.iloc[0]) * 100 if hasattr(total_returns, 'iloc') else float(total_returns) * 100
