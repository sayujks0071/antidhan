...
def main():
    ...
    # Check if total_returns has an index (it might be a scalar for a single symbol/strategy)
    if hasattr(total_returns, 'index'):
        if isinstance(total_returns.index, pd.MultiIndex):
            ...
            val = total_returns.iloc[0] if len(total_returns) > 0 else total_returns
    else:
        # Scalar value for single symbol without param arrays
