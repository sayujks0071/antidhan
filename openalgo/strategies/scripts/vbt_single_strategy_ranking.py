...
def main():
    ...
    # Handle the scalar float returned by total_return() for single-symbol
    # print(f"Returns type: {type(total_returns)}")
    # print(f"Returns: {total_returns}")

    if isinstance(total_returns, (float, np.float64)):
        ret_pct = float(total_returns) * 100
    else:
        # Fallback if not a scalar (e.g., if passing multiple symbols in the future)
