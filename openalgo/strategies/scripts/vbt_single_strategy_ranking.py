...
def main():
    ...
    if isinstance(total_return, pd.Series):
        if len(total_return) == 1:
            ret = float(total_return.iloc[0])
        else:
            ret = float(total_return.iloc[-1])
    elif isinstance(total_return, pd.DataFrame):
        ret = float(total_return.iloc[-1, -1])
    else:
        # It's likely a scalar (float) if only a single symbol and single param set is used
