...
logger = logging.getLogger("MCX_Gold_Momentum")
...
class MCXStrategy:
    ...
    def check_signals(self):
        ...
        if not has_position:
            if entry_condition:
                logger.info(f"BUY SIGNAL: Price={current['close']}, RSI={current['rsi']:.2f}, EMA_Fast={current['ema_fast']:.2f}, EMA_Slow={current['ema_slow']:.2f}")
                if self.pm:
        ...
        # Exit Logic
            ...
            if exit_condition:
                logger.info(f"EXIT: Trend Faded. Price={current['close']}, RSI={current['rsi']:.2f}")
...
if __name__ == "__main__":
    ...
    PARAMS = {
        ...
        "period_ema_fast": args.period_ema_fast,
        "period_ema_slow": args.period_ema_slow,
